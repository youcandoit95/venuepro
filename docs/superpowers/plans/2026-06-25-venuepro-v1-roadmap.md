...
- **Config & credentials (standing rule):** **every** credential/config value lives in **`.env`** (read via `config()`, never hardcoded in code). **`.env.example` is kept in lockstep** — whenever a task introduces a new env key, add it to `.env.example` in the **same commit** with a safe placeholder/empty value (NEVER a real secret). `.env` stays gitignored; `.env.example` is the committed contract of required keys. Reviewers reject any new `env()`/`config()` key missing from `.env.example`.
